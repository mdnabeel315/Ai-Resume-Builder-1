"""
LLM Service — Production-grade abstraction over Google Gemini 2.0 Flash.

Features:
  - Lazy singleton Gemini client (thread-safe, configured once)
  - Streamlit secrets → environment variable fallback for API key
  - Tenacity retry: 3 attempts, exponential back-off (2–10 s)
  - TTL cache (5 min) — identical prompts skip the API entirely
  - Circuit breaker — pauses calls after 5 consecutive failures
  - Auto-strip of markdown fences from JSON responses
...
def _get_api_key() -> str:
    """Load Gemini API key: Streamlit secrets first, then environment variable."""
    try:
...
def _ensure_client() -> None:
    """Configure the Gemini client exactly once (double-checked locking)."""
    ...
    with _client_lock:
        if not _client_configured:
            ...
            logger.info("Gemini client configured — model: %s", _MODEL_NAME)
...
_CIRCUIT_RESET_AFTER: int = 60  # seconds
_circuit_open_until: float = 0.0


def _check_circuit() -> None:
    """Raise LLMError if the circuit breaker is currently open."""
    with _cb_lock:
...
def _record_failure() -> None:
    ...
    with _cb_lock:
        ...
        if _consecutive_failures >= _MAX_CONSECUTIVE_FAILURES:
            ...
            logger.warning(
                "Circuit breaker OPEN — %d consecutive failures. Resets in %ds.",
...
# ── Raw Gemini Call (with Tenacity retry) ──────────────────────────────────────
@retry(
...
def complete(
    ...
    *,
    ...
    Args:
        ...
        use_cache:     Set False for creative/non-deterministic calls.
    ...
    Raises:
        LLMError:      On Gemini call failure after retries or open circuit.
    ...
    if use_cache:
        ...
        if cached is not None:
            logger.debug("LLM cache hit — key prefix: %s", key[:8])
            return cached
    ...
    try:
        ...
        raise LLMParseError(
            f"Could not parse LLM JSON: {exc}\n"
