"""
ZNA AI Studio — Overview Dashboard
Streamlit Cloud entry point: streamlit_app.py (repo root)
"""
...
with st.sidebar:
    ...
    if target:
        ...
        st.info("💡 Fill out **Target Job Title** in the Resume Builder to unlock the Job Portal.")
    if has_resume():
        r = get_resume()
...
with col_chart:
    ...
    if not history:
        st.caption("_Sample data — generate your first resume to start tracking._")

with col_logs:
    st.subheader("⚡ Live System Logs")

    # Safe key check (works on Streamlit Cloud AND locally)
    gemini_key = ""
    try:
    ...
    if gemini_key:
        ...
        st.error("❌ **[System]** GEMINI_API_KEY not set in Secrets.")
